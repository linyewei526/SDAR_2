...
class ExpertBufferManager:
    ...
    def __init__(
        ...
        self.last_load_trace: Dict[str, Any] = {}
    ...
    def load_experts_for_current_layer(
        ...
        self.last_load_trace = {}

        loaded_mapping = {}
        cache_expert_ids_before = []
        cache_expert_ids_after = []
        prefetch_available_expert_ids_before = []
        gpu_cache_hit_expert_ids = []
        prefetch_hit_expert_ids = []
        remaining_experts = []
        ...
        with nvtx_range(f"Current_Layer_Availability_Check_Layer{layer_idx}"):
            ...
            if self.gpu_cache_manager is not None:
                cache_expert_ids_before = sorted(
                    expert_id
                    for (cached_layer_idx, expert_id), _slot_idx in self.gpu_cache_manager.cached_experts.items()
                    if cached_layer_idx == layer_idx
                )
            prefetch_available_expert_ids_before = sorted(self.prefetch_mapping.keys())
            ...
            for expert_id in expert_ids:
                if self.gpu_cache_manager is not None:
                    ...
                    if cache_slot_idx is not None:
                        ...
                        gpu_cache_hit_expert_ids.append(expert_id)
                        continue
            ...
            for expert_id in experts_need_temp_load:
                if expert_id in self.prefetch_mapping:
                    ...
                    prefetch_hits += 1
                    prefetch_hit_expert_ids.append(expert_id)
                else:
        ...
        self.last_load_trace = {
            "layer_idx": layer_idx,
            "requested_expert_count": len(expert_ids),
            "requested_expert_ids": sorted(expert_ids),
            "gpu_cache_expert_count_before": len(cache_expert_ids_before),
            "gpu_cache_expert_ids_before": cache_expert_ids_before,
            "gpu_cache_hit_count": len(gpu_cache_hit_expert_ids),
            "gpu_cache_hit_expert_ids": sorted(gpu_cache_hit_expert_ids),
            "prefetch_available_expert_count_before": len(prefetch_available_expert_ids_before),
            "prefetch_available_expert_ids_before": prefetch_available_expert_ids_before,
            "prefetch_hit_count": len(prefetch_hit_expert_ids),
            "prefetch_hit_expert_ids": sorted(prefetch_hit_expert_ids),
            "cpu_miss_load_count": len(remaining_experts),
            "cpu_miss_load_expert_ids": sorted(remaining_experts),
            "gpu_cache_expert_count_after": len(cache_expert_ids_after),
            "gpu_cache_expert_ids_after": cache_expert_ids_after,
            "gpu_cache_replacement_count": len(
                set(cache_expert_ids_after) - set(cache_expert_ids_before)
            ),
        }
