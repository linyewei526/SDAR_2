...
def _get_benchmark_path(benchmark: str) -> Path:
    benchmark_paths = {
        "gsm8k": BENCHMARK_ROOT / "gsm8k" / "main" / "test-00000-of-00001.parquet",
        "humaneval": BENCHMARK_ROOT / "openai_humaneval" / "openai_humaneval" / "test-00000-of-00001.parquet",
        "cnndm": BENCHMARK_ROOT / "CNN-DM.parquet",
    }

    try:
        benchmark_path = benchmark_paths[benchmark]
    except KeyError as exc:
        raise ValueError(f"Unknown benchmark: {benchmark}") from exc

    if not benchmark_path.exists():
        raise FileNotFoundError(
            f"Benchmark file not found: {benchmark_path}. "
            f"Expected benchmark data under {BENCHMARK_ROOT}"
        )
...
def get_prompts(benchmark="gsm8k", num_samples=1, start_idx=0):
    ...
    if benchmark == "gsm8k":
        benchmark_path = _get_benchmark_path("gsm8k")
        ...
        benchmark_path = _get_benchmark_path("humaneval")
        table = pq.read_table(benchmark_path)
        df = table.to_pandas()
        prompts = []
        end_idx = start_idx + num_samples
        ...
        benchmark_path = _get_benchmark_path("cnndm")
        table = pq.read_table(benchmark_path)
        df = table.to_pandas()
        prompts = []
        end_idx = start_idx + num_samples
...
QWEN3MOE_CONFIG = BaselineModelConfig(
    ...
    default_model_path="/data_3/tianjianyang/models/moe/Qwen3-30B-A3B",
...
GPTOSS_CONFIG = BaselineModelConfig(
    ...
    top_k=4,
    builder_func=None,  # 将在导入后设置
    default_model_path="/data_3/tianjianyang/models/moe/gpt-oss-20b-BF16",
...
class BaselineTestBase:
    ...
    def _should_mark_nsys_capture(self, sample_number: int, args) -> bool:
        single_sample = getattr(args, "nsys_profile_single_sample", 0)
        if single_sample > 0:
            return sample_number == single_sample
        every_k = getattr(args, "nsys_profile_every_k", 0)
        first_sample = getattr(args, "nsys_profile_first_sample", 1)
        return every_k > 0 and sample_number >= first_sample and (sample_number - first_sample) % every_k == 0

    @staticmethod
    def _should_enable_nvtx_ranges(args) -> bool:
        return (
            getattr(args, "enable_nvtx_ranges", False)
            or getattr(args, "nsys_profile_single_sample", 0) > 0
            or getattr(args, "nsys_profile_every_k", 0) > 0
        )

    def _capture_cuda_memory_snapshot(
        self,
        snapshots,
        stage: str,
    ...
    def _resolve_memory_output_path(self, args) -> Path:
        if getattr(args, "gpu_memory_output", None):
            return Path(args.gpu_memory_output)

        return PROJECT_ROOT / "profiles" / (
            f"{self.config.name}_{args.benchmark}_start{args.start_idx}_n{args.num_samples}_gpu_memory.csv"
        )

    def _write_memory_snapshots(self, snapshots, args):
        if not snapshots:
            return None

        output_path = self._resolve_memory_output_path(args)
        ...
        with output_path.open("w", newline="") as csv_file:
            ...
            for snapshot in snapshots:
                ...
                for byte_field, gib_field in zip(byte_fields, gib_fields):
                    value = row.get(byte_field, "")
                    if value == "":
                        row[gib_field] = ""
                    else:
                        row[gib_field] = f"{self._bytes_to_gib(value):.6f}"
                writer.writerow(row)

        return output_path

    def _print_memory_summary(self, snapshots, output_path: Optional[Path]):
        if not snapshots:
            return

        max_device_used = max(snapshot["device_used_bytes"] for snapshot in snapshots)
        max_torch_reserved = max(
            snapshot.get("sample_peak_reserved_bytes", snapshot["torch_max_reserved_bytes"]) for snapshot in snapshots
        )
        max_torch_allocated = max(
            snapshot.get("sample_peak_allocated_bytes", snapshot["torch_max_allocated_bytes"]) for snapshot in snapshots
        )
        ...
        print(f"{'='*60}")
        print(
            f"Peak device used (sampled via cudaMemGetInfo): "
            f"{self._bytes_to_gib(max_device_used):.2f} GiB"
        )
        print(
            f"Peak torch reserved (sample/local peak): "
            f"{self._bytes_to_gib(max_torch_reserved):.2f} GiB"
        )
        print(
            f"Peak torch allocated (sample/local peak): "
            f"{self._bytes_to_gib(max_torch_allocated):.2f} GiB"
        )
        if output_path is not None:
            print(f"Memory snapshots saved to: {output_path}")
        print("Note: device_used is sampled global device usage; torch_* is this process allocator view.")
        print(f"{'='*60}\n")
    ...
    def run_test(self, args, build_model_kwargs: Optional[Dict] = None) -> bool:
        ...
        memory_snapshots = [] if getattr(args, "track_gpu_memory", False) and torch.cuda.is_available() else None
        cuda_profiler_runtime = None
        if getattr(args, "nsys_use_cuda_profiler_api", False):
            if not torch.cuda.is_available():
                raise RuntimeError("--nsys-use-cuda-profiler-api requires CUDA to be available")
            cuda_profiler_runtime = torch.cuda.cudart()

        nvtx_ranges_enabled = self._should_enable_nvtx_ranges(args)
        set_nvtx_enabled(nvtx_ranges_enabled)

        print(f"🔧 Building {self.config.name} model with offloading...")
        nsys_single_sample = getattr(args, "nsys_profile_single_sample", 0)
        nsys_every_k = getattr(args, "nsys_profile_every_k", 0)
        if nsys_single_sample > 0 or nsys_every_k > 0:
            if cuda_profiler_runtime is not None:
                if nsys_single_sample > 0:
                    print(
                        "🎯 Nsight capture markers enabled: "
                        f"sample {nsys_single_sample} only, trigger = cudaProfilerStart/Stop"
                    )
                else:
                    print(
                        "🎯 Nsight capture markers enabled: "
                        f"every {nsys_every_k} sample(s) starting from sample {args.nsys_profile_first_sample}, "
                        "trigger = cudaProfilerStart/Stop"
                    )
            else:
                if nsys_single_sample > 0:
                    print(
                        "🎯 Nsight capture markers enabled: "
                        f"sample {nsys_single_sample} only, range name = {args.nsys_capture_range_name}"
                    )
                else:
                    print(
                        "🎯 Nsight capture markers enabled: "
                        f"every {nsys_every_k} sample(s) starting from sample {args.nsys_profile_first_sample}, "
                        f"range name = {args.nsys_capture_range_name}"
                    )
        elif nvtx_ranges_enabled:
            print("🏷️ NVTX ranges enabled")
        else:
            print("🏷️ NVTX ranges disabled for pure baseline run")
        if memory_snapshots is not None:
            print("📈 GPU memory snapshot logging enabled")
            self._capture_cuda_memory_snapshot(
                memory_snapshots,
                stage="pre_build",
                relative_time_s=time.perf_counter() - benchmark_start_time,
            )
        ...
        with nvtx_range("Benchmark_Run"):
            # 串行处理每个 prompt (bsz=1)
            for sample_idx, prompt in enumerate(prompts):
                sample_number = sample_idx + 1
                sample_name = f"Sample_{sample_number}"
                mark_nsys_capture = self._should_mark_nsys_capture(sample_number, args)

                print(f"\n{'='*60}")
                print(f"🔄 Processing Sample {sample_number}/{len(prompts)}")
                print(f"{'='*60}")
                ...
                try:
                    if mark_nsys_capture and cuda_profiler_runtime is not None:
                        cuda_profiler_runtime.cudaProfilerStart()
                        profiler_started = True

                    with ExitStack() as stack:
                        if mark_nsys_capture and cuda_profiler_runtime is None:
                            stack.enter_context(nvtx_range(args.nsys_capture_range_name))
                        stack.enter_context(nvtx_range(f"{sample_name}_Total"))
                        ...
                        with torch.no_grad():
                            # Prefill 阶段
                            with nvtx_range(f"{sample_name}_Prefill"):
                            ...
                            with nvtx_range(f"{sample_name}_Decode"):
                ...
                if memory_snapshots is not None:
                    sample_peak_allocated = torch.cuda.max_memory_allocated()
                    sample_peak_reserved = torch.cuda.max_memory_reserved()
                    self._capture_cuda_memory_snapshot(
                        memory_snapshots,
                        stage="sample_end",
                        sample_idx=sample_number,
                ...
                sample_msg = f"✅ Sample {sample_number} completed: {new_tokens} tokens, {decode_tps:.2f} tokens/s"
                if memory_snapshots is not None:
                    sample_msg += f", peak reserved {self._bytes_to_gib(sample_peak_reserved):.2f} GiB"
                print(sample_msg)
        ...
        memory_output_path = self._write_memory_snapshots(memory_snapshots, args)
        self._print_memory_summary(memory_snapshots, memory_output_path)
...
def add_baseline_common_args(parser):
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="If >0, mark every k-th sample for Nsight Systems capture",
    )
    parser.add_argument(
        "--nsys-profile-first-sample",
        type=int,
        default=1,
        help="1-based local sample index of the first marked Nsight capture sample",
    )
    parser.add_argument(
        "--nsys-profile-single-sample",
        type=int,
        default=0,
        help="If >0, mark only this 1-based local sample index for Nsight capture",
    )
    parser.add_argument(
        "--nsys-capture-range-name",
        type=str,
        default="Nsys_Profile_Capture",
        help="NVTX range name used together with nsys --capture-range=nvtx",
    )
    parser.add_argument(
        "--nsys-use-cuda-profiler-api",
        action="store_true",
        help="Use cudaProfilerStart/Stop on marked samples for Nsight capture",
    )
    parser.add_argument(
        "--enable-nvtx-ranges",
        action="store_true",
        help="Emit NVTX ranges even without Nsight capture markers",
    )
    parser.add_argument(
        "--track-gpu-memory",
        action="store_true",
        help="Record coarse GPU memory snapshots and per-sample peaks to CSV",
    )
    parser.add_argument(
        "--gpu-memory-output",
        type=str,
        default=None,
        help="Optional CSV path for GPU memory snapshots",
    )
    return parser
